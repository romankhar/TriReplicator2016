...
/**
 * This class allows one to get and put data into the USAT NCC website:
 * http://www.race-tracker.net/usat/admin/add_indresults.cfm The USAT site does
 * not have an API, so we are using screen scraping technology and HTMLUnit to
 * interact with the site - this could be broken easily when USAT changes their
 * site layout...
 * 
 * @author Roman Kharkovski, http://kharkovski.blogspot.com
 */
public class USATclient implements TrainingAPI {
	private static final Logger log = Utils.getLogger(USATclient.class.getName());

	public String hostName = "http://ncc.usatriathlon.org";
	public String entryURL = hostName + "/home/log";
	public String logoutUrl = hostName;
	...
	public void setupTrainingLog(String username, String password) throws TrainingLogException {
		log.finest("--- setupTrainingLog()");
		setPassword(password);
		setUsername(username);
	}

	/**
	 * Add user workouts to USAT site by iterating over several pages of the
	 * site and using HTMLUnit in the process of doing so
	 */
	@Override
	public List<WorkoutSession> addWorkouts(List<WorkoutSession> inputSessions) throws TrainingLogException {
		log.finest("--- addWorkouts()");
		// Return null if input is empty
		if ((inputSessions == null) || (inputSessions.size() == 0)) {
			log.finest("Nothing to add to the list of workouts as it is empty");
			return null;
		}

		// This will be a collection of all workouts that made it to the USAT
		// site successfully
		List<WorkoutSession> addedSessions = new ArrayList<WorkoutSession>();

		// Prepare input content for entry into the site.
		// USAT page can only accept one workout per page
		...
		while (workoutIterator.hasNext()) {
			...
			try {
				if (session.isValidWorkout()) {
					...
					log.info("The workout was found to be invalid (some values out of range, etc.). Workout = "
							+ session.toString());
				}
			} catch (Exception e) {
				// If there was an error with one of the workouts - skip it and
				// move on to the next workout
				log.info("There was an error adding the workout: " + e.getMessage());
			}
		}
	...
	private void createBrowser() {
		log.finest("--- Creating new webClient");
		// set the log level: "trace", "debug", "info", "warn", "error", or
		// "fatal"
	...
	private void doLoginPage() throws TrainingLogException {
		...
		// log.finest("--- doLoginPage");
		try {
			loginPage = webClient.getPage(entryURL);
			// log.finest( loginPage.asText());
		...
		// HtmlTextInput userName = form.getInputByName("MemberId");
		@SuppressWarnings("deprecation")
		HtmlTextInput userName = form.getElementById("MemberId");
		userName.setValueAttribute(usatUser);
		// HtmlPasswordInput password = form.getInputByName("password");
		@SuppressWarnings("deprecation")
		HtmlPasswordInput password = form.getElementById("password");
		...
		// HtmlSubmitInput submitLogin = form.getInputByValue("Log in");
		HtmlButton submitButton = (HtmlButton) form.getElementsByTagName("button").get(0);
		...
		afterLoginPage = null;
		try {
			afterLoginPage = submitButton.click();
			// log.finest("-------------------------------------------------------------------------------------------");
			// log.finest(afterLoginPage.asText());
			if (afterLoginPage.asText().contains("The user name or password is incorrect"))
				throw new TrainingLogException(
						"Can not login into the USAT NCC site - please check your user ID and password");
		} catch (IOException e1) {
			e1.printStackTrace();
			throw new TrainingLogException(
					"Network error - Can not login into the USAT NCC site - please check your user ID and password",
					e1);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void doStartEnterResultsPage() throws TrainingLogException {
		log.finest("--- doStartEnterResultsPage()");
		log.finest("---------------------------");
		log.finest(afterLoginPage.asXml());
		// Just to make sure we have the right page, lets find something unique
		// there that indicates we can enter results
		if (afterLoginPage.asText().indexOf("Please enter your training details below") > -1) {
			enterWorkoutPage = afterLoginPage;
		} else {
			throw new TrainingLogException("Could not find the HTML page to enter workout details");
		}

		// } catch (IOException e1) {
		// e1.printStackTrace();
		// String errorMsg = "There is a problem with USAT site - after Login
		// page";
		// log.info(errorMsg);
	...
	private void doLogoutPage() {
		log.finest("--- doLogoutPage()");

		try {
			@SuppressWarnings("unused")
			HtmlPage logoutPage = webClient.getPage(logoutUrl);
			// log.finest( logoutPage.asText());
		} catch (FailingHttpStatusCodeException e1) {
			e1.printStackTrace();
			log.info("Can not logout due to an error with USAT site");
			// throw new TrainingLogException("Can not logout due to an error
			// with USAT site", e1);
		} catch (MalformedURLException e1) {
			e1.printStackTrace();
			log.info("Wrong URL for USAT site when getting logout page");
			// throw new TrainingLogException("Wrong URL for USAT site when
			// getting logout page", e1);
		} catch (IOException e1) {
			e1.printStackTrace();
			log.info("Network error when getting USAT logout page");
			// throw new TrainingLogException("Network error when getting USAT
			// logout page", e1);
		}
	}

	/**
	 * This method adds three workouts to the website (one of each kind - swim,
	 * bike, run) and returns the next page to be browsed and also updates the
	 * list of sessions that were added in the addedSessions parameter
	 * 
	...
	private boolean doEnterResultsPage(WorkoutSession session) throws TrainingLogException {
		...
		if (!pageText.contains(new SimpleDateFormat("MMMMM").format(session.getWorkoutDate()))
				&& !pageText.contains(new SimpleDateFormat("MMM").format(session.getWorkoutDate()))) {
			log.finest("*********** The workout falls outside of the allowed date range: " + session.toString());
			return false;
		}
		...
		if (session.getWorkoutType() == WorkoutType.Swim) {
			...
			distance.setValueAttribute(Long.toString(session.getWorkoutDistanceYards())); // minimum
																							// yards
																							// is
																							// 50
		...
		if (session.getWorkoutType() == WorkoutType.Bike) {
			...
			distance.setValueAttribute(Double.toString(session.getWorkoutDistanceMiles())); // always
																							// in
																							// miles
																							// only
		...
		if (session.getWorkoutType() == WorkoutType.Run) {
			...
			runType.setDefaultChecked(true); // this sets it to regular "run"
												// (as opposed to Elliptical or
												// x-country
												// skiing)
		...
		if (dateField == null)
			// this would mean that the workout is of none of the types
			// specified above, so we abort
			return false;
		...
		while (datesIterator.hasNext()) {
			...
			if (htmlOption.asText().contentEquals(dayOfTheMonth)) {
				// If at least one date corresponding to the workout date is
				// found in options, we are OK to proceed
				dateFound = true;
			}
			// log.finest( htmlOption.asText());
		}
		dateField.setSelectedAttribute(dayOfTheMonth, true);

		if (!dateFound) {
			log.finest(
					"We have not found a date for the workout in the allowed date range, hence exit without saving anything. Workout = "
							+ session.toString());
			return false;
		}
		...
		try {
			...
			e.printStackTrace();
			throw new TrainingLogException(
					"The workout could not be added - perhaps it has a wrong date or something else went wrong", e);
		}
		...
		if (confirmWorkoutPage.asText().contains("Please verify your entry before saving")) {
			// Confirmation page needs to have this text to make sure workout is
			// being added correctly
			return true;
		} else {
			log.info("Could not confirm that the workout was added - reason is unknown. Workout = "
					+ session.toString());
			return false;
		}
	}
	...
	private boolean doConfirmResultsPage() throws TrainingLogException {
		log.finest("--- doConfirmResultsPage()");
		// log.finest( confirmWorkoutPage.asText());
		...
		try {
			...
			log.finest(" SHOW CONFIRMED RESULTS PAGE");
			// log.finest( reviewWorkoutsPage.asXml());
			// log.finest( reviewWorkoutsPage.asText());
			...
			if (reviewWorkoutsPage.asText().contains("Results added successfully")) {
				...
				log.finest("doConfirmResultsPage: Could not confirm that the workout was added");
				return false;
			}
		} catch (IOException e1) {
			e1.printStackTrace();
			log.finest("doConfirmResultsPage: Could not confirm that the workout was added - an IO error occured");
